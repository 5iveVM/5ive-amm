...
use five_vm_mito::{
    error::VMErrorCode, stack::StackStorage, systems::resource::ResourceManager, AccountInfo,
    MitoVM, FIVE_VM_PROGRAM_ID,
};
...
fn test_resource_manager_heap_tracking() {
    ...
    assert_eq!(
        manager.heap_usage(),
        2048,
        "Initial heap usage should be 2048 (initial chunk)"
    );
    ...
    assert_eq!(
        manager.heap_usage(),
        2048,
        "Heap usage should reflect first chunk size"
    );
    ...
    assert_eq!(
        manager.heap_usage(),
        2048,
        "Heap usage should unchanged when reusing chunk"
    );
    ...
    assert_eq!(
        manager.heap_usage(),
        2048 + large_size,
        "Heap usage should increase by large allocation size"
    );
}
...
fn test_call_external_returns_call_stack_overflow_on_locals_window_overflow() {
    ...
    main_script.extend_from_slice(&[
        ALLOC_LOCALS,
        31, // Set current local window close to MAX_LOCALS (32)
        CALL_EXTERNAL,
        0, // account index
        FIVE_HEADER_OPTIMIZED_SIZE as u8,
        0, // external function offset
    ...
    assert!(
        matches!(
            error_code,
            VMErrorCode::CallStackOverflow | VMErrorCode::StackError
        ),
