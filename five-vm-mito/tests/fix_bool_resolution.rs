...
use five_vm_mito::{utils, ExecutionContext, StackStorage, ValueRef};
...
fn test_resolve_bool_from_account_ref() {
    ...
    assert_eq!(
        result.unwrap(),
        true,
        "resolve_bool should return true for byte 1"
    );
    ...
    assert_eq!(
        result_false.unwrap(),
        false,
        "resolve_bool should return false for byte 0"
    );
    ...
    assert_eq!(
        result_non_zero.unwrap(),
        true,
        "resolve_bool should return true for byte 2"
    );
}
