...
#[cfg(test)]
mod bytes_literal_tests {
    use super::*;

    #[test]
    fn typed_u8_literal_31_bytes_executes_without_oom() {
        let source = r#"
pub probe() -> u64 {
    let payload: [u8; 31] = [
        0, 1, 2, 3, 4, 5, 6, 7,
...
"#;
        ...
        let bytecode = DslCompiler::compile_dsl(source).expect("compile 31-byte literal");
        let result = execute_test(&bytecode, &[], &[]).expect("execute 31-byte literal");
        assert_eq!(result, Some(Value::U64(1)));
    }

    #[test]
    fn typed_u8_literal_64_bytes_executes_without_oom() {
        let source = r#"
pub probe() -> u64 {
    let payload: [u8; 64] = [
        255, 0, 1, 2, 3, 4, 5, 6,
        ...
        55, 56, 57, 58, 59, 60, 0, 254
    ];
    return 1;
}
"#;

        let bytecode = DslCompiler::compile_dsl(source).expect("compile 64-byte literal");
        let result = execute_test(&bytecode, &[], &[]).expect("execute 64-byte literal");
        assert_eq!(result, Some(Value::U64(1)));
    }
}
