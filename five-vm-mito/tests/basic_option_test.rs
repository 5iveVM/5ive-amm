...
fn test_basic_option_creation() {
    ...
    let result = MitoVM::execute_direct(&script, &[], &[], &FIVE_VM_PROGRAM_ID, &mut storage)
        .expect("VM should execute option bytecode");
...
fn test_basic_result_creation() {
    ...
    let mut storage = StackStorage::new();
    let result = MitoVM::execute_direct(&script, &[], &[], &FIVE_VM_PROGRAM_ID, &mut storage)
        .expect("VM should execute result bytecode");
