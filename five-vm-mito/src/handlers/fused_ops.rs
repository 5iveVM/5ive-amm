...
pub fn handle_fused_ops(opcode: u8, ctx: &mut ExecutionManager) -> CompactResult<()> {
    match opcode {
        ...
        LOAD_FIELD_REG => {
            ...
            let val = u64::from_le_bytes(data[offset as usize..offset as usize + 8].try_into().unwrap());
            ctx.set_register(reg, five_protocol::ValueRef::U64(val))?;
            debug_log!("MitoVM: LOAD_FIELD_REG reg={} acc={} offset={} val={}", reg, acc_idx, offset, val);
        }
        ...
        REQUIRE_GTE_REG => {
            ...
            let val1 = ctx.get_register(src1)?.as_u64().ok_or(VMErrorCode::TypeMismatch)?;
            let val2 = ctx.get_register(src2)?.as_u64().ok_or(VMErrorCode::TypeMismatch)?;

            if val1 < val2 {
                debug_log!("MitoVM: REQUIRE_GTE_REG failed: {} < {}", val1, val2);
                return Err(VMErrorCode::ConstraintViolation);
            }
            debug_log!("MitoVM: REQUIRE_GTE_REG passed: {} >= {}", val1, val2);
        }
        ...
        STORE_FIELD_REG => {
            ...
            let val = ctx.get_register(reg)?.as_u64().ok_or(VMErrorCode::TypeMismatch)?;
            let account = ctx.get_account_for_write(acc_idx)?;
            let data = unsafe { account.borrow_mut_data_unchecked() };
            ...
            data[offset as usize..offset as usize + 8].copy_from_slice(&val.to_le_bytes());
            debug_log!("MitoVM: STORE_FIELD_REG reg={} acc={} offset={} val={}", reg, acc_idx, offset, val);
        }
        ...
        ADD_FIELD_REG => {
            ...
            let reg_val = ctx.get_register(reg)?.as_u64().ok_or(VMErrorCode::TypeMismatch)?;
            let account = ctx.get_account_for_write(acc_idx)?;
            let data = unsafe { account.borrow_mut_data_unchecked() };
            ...
            let current = u64::from_le_bytes(data[offset as usize..offset as usize + 8].try_into().unwrap());
            let new_val = current.saturating_add(reg_val);
            data[offset as usize..offset as usize + 8].copy_from_slice(&new_val.to_le_bytes());
            debug_log!("MitoVM: ADD_FIELD_REG acc={} offset={} reg={} ({} + {} = {})", 
        ...
        SUB_FIELD_REG => {
            ...
            let current = u64::from_le_bytes(data[offset as usize..offset as usize + 8].try_into().unwrap());
            let new_val = current.saturating_sub(reg_val);
            data[offset as usize..offset as usize + 8].copy_from_slice(&new_val.to_le_bytes());
            debug_log!("MitoVM: SUB_FIELD_REG acc={} offset={} reg={} ({} - {} = {})", 
                acc_idx, offset, reg, current, reg_val, new_val);
        }
