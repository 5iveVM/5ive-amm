...
impl DslParser {
    ...
    fn parse_init_arguments(&mut self) -> Result<(Option<String>, Option<u64>), VMError> {
        ...
        while !matches!(self.current_token, Token::RightParen | Token::Eof) {
            let key = match &self.current_token {
                Token::Identifier(name) => name.clone(),
                _ => return Err(self.parse_error("argument key in @init()")),
            };
            self.advance(); // consume key
            
            if !matches!(self.current_token, Token::Assign) {
                return Err(self.parse_error("'=' after argument key in @init"));
            }
            self.advance(); // consume '='
            
            match key.as_str() {
                "payer" => {
                    payer = match &self.current_token {
                        Token::Identifier(name) => {
                            let n = name.clone();
                            self.advance();
                            Some(n)
                        ...
                        _ => return Err(self.parse_error("payer account name")),
                    };
                }
                "space" => {
                    space = match &self.current_token {
        ...
        if !matches!(self.current_token, Token::RightParen) {
            return Err(self.parse_error("')' to close @init arguments"));
        }
        self.advance(); // consume ')'
        ...
        match &token {
            ...
            Token::LeftBracket => {
                ...
                if matches!(self.current_token, Token::Semicolon) {
                    ...
                    Err(self.parse_error("';' in array type declaration"))
                }
            }
    ...
    fn parse_interface_definition(&mut self) -> Result<AstNode, VMError> {
        eprintln!("DEBUG: parse_interface_definition start");
        ...
        eprintln!("DEBUG: Parsing interface {}", name);
        self.advance();
