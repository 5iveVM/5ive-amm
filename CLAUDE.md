...
### Benchmarking (SDK Validator Suites: localnet/devnet)

Use this as the canonical validator path. It runs Node-based suites via `FiveSDK` + `FiveProgram` (the same APIs users run in production).

Files:
- Runner: `scripts/run-sdk-validator-suites.sh`
- Shared config: `scripts/lib/sdk-validator-config.mjs`
- Shared reporting: `scripts/lib/sdk-validator-reporter.mjs`
- Report builder: `scripts/sdk-validator-report.mjs`
- Output reports: `target/sdk-validator-runs/<timestamp>/sdk-validator-report.{json,md}`
...
- Scenario summary in `sdk-validator-report.md` with PASS/FAIL/PARTIAL.
- Per-step records with:
  - `signature`
  - `computeUnits`, or
  - explicit `missingCuReason`

#### Internal benchmark-only path (non-canonical)
Rust validator harness remains available for internal VM benchmarking:
- `five-solana/tests/runtime_validator_cu_tests.rs`
- `five-solana/tests/harness/validator.rs`

It is intentionally removed from default user-facing runbooks and release gates.
