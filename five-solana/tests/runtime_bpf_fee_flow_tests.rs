...
async fn deploy_fails_when_owner_cannot_pay_deploy_fee() {
    ...
    assert_eq!(admin_after, admin_before, "admin must not receive partial fee");
}
...
async fn execute_fails_when_owner_cannot_pay_execute_fee() {
    ...
    let owner_before = ctx
        ...
        .get_account(owner.pubkey())
        .await
        .expect("owner before execute account fetch")
        .expect("owner before execute missing")
        .lamports;
    let admin_before = ctx
        .banks_client
        .get_account(admin.pubkey())
        .await
        .expect("admin before execute account fetch")
        .expect("admin before execute missing")
        .lamports;
