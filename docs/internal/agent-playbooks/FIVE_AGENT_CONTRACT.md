...
1. Prefer `param.key` for account key extraction.
...
3. Anchor CPI: use `@anchor`, do not add manual `@discriminator`.
4. Non-Anchor CPI: use single-byte `@discriminator(N)` per method.
