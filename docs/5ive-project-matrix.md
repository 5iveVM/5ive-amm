...
- Deprecated variants retained: `5ive-lending`, `5ive-lending-3`, `5ive-lending-4`
...
| `5ive-lending` (deprecated) | `npm run build` | `npm test` | No | `npm run test:onchain:local` | `npm run test:onchain:devnet` | Deprecated intentionally fails execution path |
...
| `5ive-lending-3` (deprecated) | `npm run build` | `npm test` | No | `npm run test:onchain:local` | `npm run test:onchain:devnet` | Experimental only, non-production |
| `5ive-lending-4` (deprecated) | `npm run build` | `npm test` | No | `npm run test:onchain:local` | `npm run test:onchain:devnet` | Deprecated intentionally fails execution path |
...
| `5ive-token-2` | `npm run build` | `npm test` | `client/main.ts` (+ variants) | `npm run test:onchain:local`, `npm run client:run:local` | `npm run test:onchain:devnet`, `npm run client:run:devnet` | Script-account drift between historical test files |
