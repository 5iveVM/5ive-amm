...
fn encode_execute_input(function_index: u32, params: &[serde_json::Value]) -> Vec<u8> {
    ...
    for param in params {
        match param {
            ...
            other => panic!("unsupported WASM matrix param: {}", other),
        }
    }
    ...
    match expected {
        serde_json::Value::Number(number) => {
            ...
            assert_eq!(
                ...
                "scenario {} returned unexpected WASM-layer value",
                scenario_id
            );
        }
        serde_json::Value::Bool(value) => {
        ...
        other => panic!("unsupported expected WASM value: {}", other),
    }
}

#[test]
fn wasm_matrix_uses_canonical_cli_corpus() {
    let root = repo_root();
