...
async function example2_ScriptCompilation() {
  ...
  const scriptSource: FiveScriptSource = `
...
async function example5_ParameterHandling() {
  ...
  try {
    ...
    if (compilation.success && compilation.bytecode) {
      ...
      const multiplyResult = await FiveSDK.executeLocally(
        compilation.bytecode,
      ...
      const processResult = await FiveSDK.executeLocally(
        compilation.bytecode,
...
async function example6_ErrorHandling() {
  ...
  const brokenScript: FiveScriptSource = `
    script BrokenScript {
      test() -> u64 {
  ...
  try {
    const result = await FiveSDK.executeLocally(
      new Uint8Array([1, 2, 3]), // Invalid bytecode
      0, // Function index
      [] // No parameters
    );
...
async function example7_LocalWASMExecution() {
  ...
  try {
    ...
    const quickResult = await compileAndExecuteLocally(
      scriptSource,
      'getAnswer', // Function name
      [],          // No parameters
      { debug: true, trace: true }
    );
    ...
    if (compilation.success && compilation.bytecode) {
      ...
      for (const func of functions) {
        ...
        const result = await FiveSDK.executeLocally(
          ...
          func.name,
          func.params,
          { debug: true, trace: false, computeUnitLimit: 100000 }
        );
...
export {
  ...
  example3_DeploymentInstructionGeneration,
  example4_ExecutionInstructionGeneration,
  example5_ParameterHandling,
  example6_ErrorHandling,
  example7_LocalWASMExecution,
  runAllExamples
};
