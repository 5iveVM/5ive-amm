...
export interface BuildOptions {
  debug?: boolean;
  mode?: 'local' | 'onchain';
  connection?: Connection;  // Required for onchain mode
  payer?: Keypair;          // Required for onchain mode
  cleanup?: boolean;        // Auto-cleanup after test
  fiveVMProgramId?: PublicKey;  // Five VM program ID for account owner
}
...
export interface AccountExecutionContext {
  ...
  keypairs: Map<string, Uint8Array>; // For signing
}
...
export class AccountTestFixture {
  private specs: FixtureAccountSpec[] = [];
  private stateTemplates: Map<string, AccountStateTemplate> = new Map();

  /**
   * Add a signer account (@signer constraint)
   * Used for transaction signers, authority checks
   */
  addSignerAccount(name: string, options: { description?: string } = {}): this {
    this.specs.push({
      name,
      type: 'signer',
   ...
   */
  addInitAccount(name: string, options: { description?: string } = {}): this {
    this.specs.push({
      name,
      type: 'init',
   ...
   */
  async build(options: BuildOptions = {}): Promise<CompiledFixture> {
    ...
    if (mode === 'local') {
      ...
      return this.buildOnChain(options as Required<Pick<BuildOptions, 'connection' | 'payer'>> & BuildOptions);
    } else {
   ...
   */
  private async buildLocal(options: BuildOptions = {}): Promise<CompiledFixture> {
    const accounts: GeneratedAccountMeta[] = [];
    const accountsByName = new Map<string, GeneratedAccountMeta>();
   ...
   */
  private async buildOnChain(options: Required<Pick<BuildOptions, 'connection' | 'payer'>> & BuildOptions): Promise<CompiledFixture> {
  ...
  private async createAccount(
    ...
    if (isSigner) {
      return await (AccountMetaGenerator as any)['generateSignerAccount'](
        constraints,
        { debug: options.debug }
      );
    } else {
      return await (AccountMetaGenerator as any)['generateRegularAccount'](
        constraints,
        { debug: options.debug }
      );
    }
  ...
  validateAgainstABI(
    ...
    if (this.specs.length !== requiredAccounts.length) {
      errors.push(
        `Account count mismatch: fixture has ${this.specs.length} accounts, ` +
    ...
    for (let i = 0; i < Math.min(this.specs.length, requiredAccounts.length); i++) {
      ...
      if (spec.type === 'signer' && !abiAccount.signer) {
        warnings.push(
          `Account ${i} (${spec.name}): marked as signer but ABI doesn't require it`
        );
      }
      if (spec.type !== 'signer' && abiAccount.signer) {
        errors.push(
          `Account ${i} (${spec.name}): ABI requires signer, but fixture provides ${spec.type}`
        );
      }
      ...
      if (isWritable && !abiAccount.writable) {
        warnings.push(
          `Account ${i} (${spec.name}): marked as writable but ABI doesn't require it`
        );
      }
      if (!isWritable && abiAccount.writable) {
        errors.push(
          `Account ${i} (${spec.name}): ABI requires writable, but fixture provides ${spec.type}`
        );
      }
    }
  ...
  getSummary(): string {
    ...
    lines.push(
      `Signers: ${this.specs.filter(s => s.type === 'signer').length}, ` +
...
export class AccountTestExecutor {
  ...
  static bindFixture(
    ...
    const keypairs = new Map<string, Uint8Array>();
