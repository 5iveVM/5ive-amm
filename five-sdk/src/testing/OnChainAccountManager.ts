...
export class OnChainAccountManager {
  private createdAccounts: PublicKey[] = [];
  private signers: Map<string, Keypair> = new Map();

  constructor(
    private connection: Connection,
    private payer: Keypair,
    private options: OnChainAccountManagerOptions = {}
  ) {
  ...
  async createSignerAccount(
    lamports: number = LAMPORTS_PER_SOL // 1 SOL default
  ...
  async createAccount(
    ...
    if (this.options.debug) {
      console.log(
        `[OnChainAccountManager] Creating account: ${publicKey.toString()} ` +
  ...
  async createPDAAccount(
    ...
    if (this.options.debug) {
      console.log(
        `[OnChainAccountManager] Creating PDA: ${pdaAddress.toString()} ` +
        `(bump=${pda.bump})`
      );
    }
    ...
    // Create PDA account
    const createdAddress = await this.createAccount(
      space,
      owner || programId,
      lamports
    );
  ...
  async writeAccountData(
    ...
    if (this.options.debug) {
      console.log(
        `[OnChainAccountManager] Writing ${data.length} bytes to account: ${accountAddress.toString()}`
      );
    }
   ...
   */
  async checkAccountExists(publicKey: PublicKey): Promise<boolean> {
    try {
   ...
   * Ensure payer has sufficient balance
   */
  async ensureSufficientBalance(required: number): Promise<void> {
    ...
    if (balance < totalRequired) {
      throw new Error(
        `Insufficient balance: ${(balance / LAMPORTS_PER_SOL).toFixed(4)} SOL available, ` +
   ...
   */
  async createAccountWithRetry(
    createFn: () => Promise<PublicKey>,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt === maxRetries) {
          throw new Error(
            `Failed to create account after ${maxRetries} attempts: ${errorMessage}`
          );
        }

        if (this.options.debug) {
          console.log(
            `[OnChainAccountManager] Account creation failed (attempt ${attempt}/${maxRetries}), ` +
  ...
  async cleanup(): Promise<void> {
    ...
    if (this.options.debug) {
      console.log(
        `[OnChainAccountManager] Cleaning up ${this.createdAccounts.length} accounts`
      );
    }
    ...
    for (const accountAddress of this.createdAccounts) {
      try {
        ...
        if (accountInfo) {
          ...
          if (accountInfo.owner.equals(SystemProgram.programId)) {
            ...
            if (signer) {
              ...
              if (this.options.debug) {
                console.log(
                  `[OnChainAccountManager] Closed account: ${accountAddress.toString()}`
                );
              }
            }
          }
        ...
        if (this.options.debug) {
          console.log(
            `[OnChainAccountManager] Error closing account ${accountAddress.toString()}: ` +
            `${errorMessage}`
          );
        }
      }
