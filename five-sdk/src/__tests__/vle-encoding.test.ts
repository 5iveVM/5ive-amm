...
describe('VLE Encoder', () => {
  ...
  describe('SDK Integration', () => {
    it('should produce correct transaction data for execute instruction', async () => {
      ...
      const instructionData = (FiveSDK as any).encodeExecuteInstruction(functionIndex, encodedParams);
    ...
    it('should handle function index encoding correctly', async () => {
      ...
      const vle0 = (FiveSDK as any).encodeVLE(0);
      const vle1 = (FiveSDK as any).encodeVLE(1);
      const vle127 = (FiveSDK as any).encodeVLE(127);
  ...
  describe('Parameter Encoding', () => {
    ...
    it('should handle empty parameters consistently', async () => {
      ...
      for (const testCase of cases) {
        const result = await VLEEncoder.encodeExecuteVLE(
          0, 
          testCase.params as any, 
          testCase.values as any
        );
    ...
    it('should encode string parameters with typed marker', async () => {
      const parameters = [{ name: 'memo', type: 'string' }];
      const values = { memo: 'hi' };

      const result = await VLEEncoder.encodeExecuteVLE(0, parameters, values);

      // Expected: [VLE(128), VLE(1), type_id(11), VLE(2), 'h', 'i']
      expect(Array.from(result)).toEqual([0x80, 0x01, 0x01, 0x0b, 0x02, 0x68, 0x69]);
    });
  });
  ...
  describe('Error Cases', () => {
    it('should handle missing parameter values gracefully', async () => {
      ...
      await expect(
        VLEEncoder.encodeExecuteVLE(0, parameters, values)
      ).rejects.toThrow(/Missing value for parameter/);
    });
  });
});
