 ...
 *
 * Comprehensive tests for the input validation system to ensure all security
 * and resource exhaustion protections are working correctly.
 */
import { describe, it, expect, beforeEach, fail } from '@jest/globals';
import { InputValidator, ValidationError, ValidationErrorType, DEFAULT_VALIDATION_CONFIG, Validators } from '../../validation/index.js';
describe('Input Validation Framework', () => {
    ...
    describe('Bytecode Validation', () => {
        ...
        it('should reject non-Uint8Array input', () => {
            const notBytecode = [1, 2, 3, 4, 5, 6, 7, 8, 9];
    ...
    describe('Options Validation', () => {
        ...
        it('should validate specific option fields', () => {
            ...
            expect(() => validator.validateOptions({ debug: 'true' })).toThrow(ValidationError);
            expect(() => validator.validateOptions({ debug: 'true' })).toThrow(/debug must be boolean/);
            // Test invalid computeUnitLimit field  
            expect(() => validator.validateOptions({ computeUnitLimit: 'high' })).toThrow(ValidationError);
            expect(() => validator.validateOptions({ computeUnitLimit: 'high' })).toThrow(/Expected number/);
            // Test invalid maxSize field
            expect(() => validator.validateOptions({ maxSize: 'large' })).toThrow(ValidationError);
            expect(() => validator.validateOptions({ maxSize: 'large' })).toThrow(/Expected number/);
        });
    });
    describe('Validators Shortcuts', () => {
        ...
        it('should throw appropriate errors from shortcuts', () => {
            expect(() => Validators.sourceCode(123)).toThrow(ValidationError);
            expect(() => Validators.bytecode([])).toThrow(ValidationError);
            expect(() => Validators.filePath('../../../etc/passwd')).toThrow(ValidationError);
            expect(() => Validators.parameters('not-array')).toThrow(ValidationError);
            expect(() => Validators.accounts('not-array')).toThrow(ValidationError);
            expect(() => Validators.functionRef([])).toThrow(ValidationError);
            expect(() => Validators.options(123)).toThrow(ValidationError);
        });
    });
    describe('Error Details', () => {
        it('should provide detailed error information', () => {
            try {
                validator.validateSourceCode(123);
            }
            catch (error) {
