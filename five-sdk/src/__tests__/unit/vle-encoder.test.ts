...
function setupWasmMock(expectedOutput: number[]): void {
...
describe('Five SDK VLE Encoder', () => {
  ...
  describe('edge cases', () => {
    ...
    it('should handle large parameter counts', async () => {
      const functionIndex = 0;
      const paramDefs = Array.from({ length: 10 }, (_, i) => ({ // Reduced to 10 for manageable test
        name: `param_${i}`,
        type: 'u64'
      }));
      const paramValues = Object.fromEntries(
        paramDefs.map((param, i) => [param.name, i * 100])
      );
