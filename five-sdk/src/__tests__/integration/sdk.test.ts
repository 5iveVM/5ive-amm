...
describe('Five SDK Integration Tests', () => {
  ...
  describe('Local WASM Execution Workflow', () => {
    ...
    it('should compile and execute in one step', async () => {
      ...
      const result = await FiveSDK.compileAndExecuteLocally(
        sourceCode,
        'add',
        [20, 22],
  ...
  describe('Deployment Instruction Generation', () => {
    it('should generate deployment instruction with PDA and rent calculation', async () => {
      ...
      const result = await FiveSDK.generateDeployInstruction(
        bytecode,
        deployer,
        { 
  ...
  describe('Execution Instruction Generation', () => {
    it('should generate execution instruction with parameter encoding', async () => {
      ...
      const result = await FiveSDK.generateExecuteInstruction(
        ...
        parameters,
        accounts,
        { debug: true, computeUnitLimit: 100000 }
      );
    ...
    it('should resolve function name to index', async () => {
      ...
      const result = await FiveSDK.generateExecuteInstruction(
        scriptAccount,
        functionIndex, // Use numeric index
        parameters,
        accounts
      );
    ...
    it('should handle missing function error', async () => {
      ...
      await expect(FiveSDK.generateExecuteInstruction(
        scriptAccount,
        functionName,
        [],
        []
      )).rejects.toThrow(`Function '${functionName}' not found`);
    });
  });
  ...
  describe('Script Metadata Operations', () => {
    ...
    it('should get script metadata with connection', async () => {
      ...
      const result = await FiveSDK.getScriptMetadataWithConnection(
        scriptAccount,
        mockConnection
      );
    ...
    it('should use cached metadata for performance', async () => {
      ...
      const result1 = await FiveSDK.getCachedScriptMetadata(
        scriptAccount,
        mockConnection,
        60000 // 1 minute TTL
      );
      ...
      const result2 = await FiveSDK.getCachedScriptMetadata(
        scriptAccount,
        mockConnection,
        60000
      );
