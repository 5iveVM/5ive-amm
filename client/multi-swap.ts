...
type SwapPlan = { dir: 'A2B' | 'B2A'; amountIn: bigint };
type CsvRow = {
  idx: number;
  dir: 'A2B' | 'B2A';
  amountIn: bigint;
...
function buildSwapPlan(): SwapPlan[] {
  ...
  if (numSwaps === 6 && !process.env.NUM_SWAPS) {
    return [
      { dir: 'A2B', amountIn: 300_000n },
      { dir: 'A2B', amountIn: 900_000n },
      { dir: 'A2B', amountIn: 1_500_000n },
      { dir: 'B2A', amountIn: 500_000n },
      { dir: 'B2A', amountIn: 700_000n },
      { dir: 'A2B', amountIn: 400_000n },
    ];
  }
  ...
  for (let i = 0; i < numSwaps; i++) {
    ...
    const amount = 100_000 + (next() % 900_001); // [100k, 1,000,000]
    swaps.push({ dir, amountIn: BigInt(amount) });
  }
  return swaps;
}

function quoteOut(amountIn: bigint, reserveIn: bigint, reserveOut: bigint) {
...
async function main() {
  ...
  for (let i = 0; i < swaps.length; i++) {
    ...
    if (actualReserveIn !== expectedReserveIn || actualReserveOut !== expectedReserveOut) {
      throw new Error(
        `swap ${i + 1}: reserve mismatch expected_in=${expectedReserveIn} actual_in=${actualReserveIn} expected_out=${expectedReserveOut} actual_out=${actualReserveOut}`
      );
    }
    if (kAfter < kBefore) {
    ...
    console.log(
      `${pad(step.name)} | sig=${step.signature} | cu=${step.computeUnits ?? 'n/a'} | in=${s.amountIn} | out=${quote.amountOut} | k_before=${kBefore} | k_after=${kAfter} | fee_a=${protocolFeesA} | fee_b=${protocolFeesB}`
    );
    csvRows.push({
      idx: i + 1,
      dir: s.dir,
  ...
  for (const row of csvRows) {
    console.log(
      `${row.idx},${row.dir},${row.amountIn},${row.amountOut},${row.cu ?? ''},${row.kBefore},${row.kAfter},${row.signature}`
    );
  }
  console.log('END_CSV');
}
