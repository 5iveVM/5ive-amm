...
async function assertTokenDelta(connection, account, before, expectedDelta, label) {
...
async function main() {
    ...
    const addA = 1000000n;
    const addB = 1000000n;
    const removeLp = 1000000n;
    const swapIn = 1000000n;
    const lpUser = Keypair.generate();
    const trader = Keypair.generate();
    setup.push(await sendSystemTx(connection, payer, SystemProgram.transfer({ fromPubkey: payer.publicKey, toPubkey: lpUser.publicKey, lamports: 30000000 }), [], 'setup:fund_lp_user'));
    setup.push(await sendSystemTx(connection, payer, SystemProgram.transfer({ fromPubkey: payer.publicKey, toPubkey: trader.publicKey, lamports: 30000000 }), [], 'setup:fund_trader'));
    ...
    await mintTo(connection, payer, mintA, lpUserAtaA.address, payer, 20000000);
    await mintTo(connection, payer, mintB, lpUserAtaB.address, payer, 20000000);
    await mintTo(connection, payer, mintA, traderAtaA.address, payer, 10000000);
