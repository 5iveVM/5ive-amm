...
impl AccountInfo {
    ...
    /// Construct an `AccountInfo` backed by heap memory (useful for tests).
    pub fn new(
        key: &Pubkey,
        is_signer: bool,
        is_writable: bool,
        ...
        unsafe {
            ...
            let data_ptr = (raw as *mut u8).add(header);
            copy_nonoverlapping(data.as_ptr(), data_ptr, data_len);
    ...
    /// Refresh this AccountInfo after CPI operations that may have modified the account.
    ///
    /// While Pinocchio's pointer calculations are usually correct even after CPI
    /// (because they're computed at access time), this method documents best practice
    /// and provides a hook for future runtime-aware optimizations.
    ///
    /// This is particularly important after:
    /// - CreateAccount (resizes account from 0 to N bytes)
    /// - Realloc (changes account data size)
    /// - Other instructions that modify account structure
    ///
    /// # Example
    ///
    /// ```ignore
    /// // Create a new account via CPI
    /// invoke(&create_account_instruction, &[payer, new_account, system])?;
    ///
    /// // Refresh pointer before writing to the new account
    /// new_account.refresh_after_cpi();
    ///
    /// // Now safe to write
    /// let data = new_account.borrow_mut_data_unchecked();
    /// data.copy_from_slice(&[1, 2, 3, 4]);
    /// ```
    ///
    /// # Explanation
    ///
    /// When the Solana runtime processes a CPI, it updates the Account struct metadata
    /// (particularly `data_len`) to reflect any size changes. Pinocchio's data access
    /// methods (`borrow_data_unchecked`, etc.) read `data_len` at call time, so they
    /// automatically see the updated value.
    ///
    /// This method is primarily a documentation and assertion point. It ensures:
    /// 1. Developers are explicit about CPI effects on accounts
    /// 2. Code reviewers see when account pointers may change
    /// 3. Future Solana runtime improvements can add validation here
    ///
    /// # Safety
    ///
    /// This method assumes the raw pointer still points to a valid Account structure
    /// in the runtime's input buffer. It should be called immediately after CPI
    /// operations that could affect the account.
    #[inline(always)]
    pub fn refresh_after_cpi(&self) {
        // No-op at this level. The Account struct metadata in the input buffer
        // is maintained by the Solana runtime and updated after CPI operations.
        // Subsequent data accesses will automatically use the current data_len.
        //
        // This method serves as:
        // 1. Documentation of CPI-safety practices
        // 2. A hook for future pointer validation
        // 3. A place to add checks if Solana runtime behavior changes
    }
}
